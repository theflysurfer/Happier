...
// Pre-configured account secret for personal APK (bypasses QR code)
// Set via EXPO_PUBLIC_HARDCODED_SECRET in EAS build secrets or .env
const HARDCODED_SECRET = process.env.EXPO_PUBLIC_HARDCODED_SECRET || '';

// Device restriction: only Julien's Samsung S22 Ultra and Dell XPS 15
async function isAllowedDevice(): Promise<boolean> {
    // Web/Tauri = Dell XPS 15 desktop
    if (Platform.OS === 'web') {
        return true;
    }
    // Android: check for Samsung S22 Ultra (model contains "SM-S908") or emulator
    if (Platform.OS === 'android') {
        const model = getModel();
        const modelLower = model.toLowerCase();
        console.log('[DeviceCheck] Android model:', model);
        if (model.includes('SM-S908') || modelLower.includes('s22 ultra')) {
            return true;
        }
        // Allow Android emulators for testing
        if (modelLower.includes('sdk') || modelLower.includes('emulator') || modelLower.includes('generic') || modelLower.includes('android sdk')) {
            console.log('[DeviceCheck] Emulator detected, allowing auto-login');
            return true;
        }
    }
    // iOS: also allow (in case of future use)
    if (Platform.OS === 'ios') {
        return true;
    }
    console.warn('[DeviceCheck] Unauthorized device:', getModel());
    return false;
}
    ...
    React.useEffect(() => {
        (async () => {
            try {
                ...
                // Auto-login with hardcoded secret if no credentials stored (device-restricted)
                if (!credentials && HARDCODED_SECRET) {
                    const allowed = await isAllowedDevice();
                    if (allowed) {
                        try {
                            const normalizedKey = normalizeSecretKey(HARDCODED_SECRET);
                            const secretBytes = decodeBase64(normalizedKey, 'base64url');
                            const token = await authGetToken(secretBytes);
                            if (token) {
                                await TokenStorage.setCredentials({ token, secret: normalizedKey });
                        ...
                        console.warn('Auto-login blocked: unauthorized device');
                    }
                }
