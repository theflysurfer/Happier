...
/**
 * Check if Plannotator is installed on a machine
 */
export async function machineCheckPlannotator(machineId: string): Promise<{
    installed: boolean;
    version?: string;
    error?: string;
}> {
    try {
        const result = await machineBash(machineId, 'plannotator --version 2>/dev/null || echo "not-installed"', '/tmp');
        if (result.success && result.stdout.trim() !== 'not-installed') {
...
/**
 * Install Plannotator on a remote machine for Claude Code plan review integration
 * Uses the official install script from plannotator.ai
 */
export async function machineInstallPlannotator(machineId: string): Promise<{
    success: boolean;
    message: string;
    alreadyInstalled?: boolean;
}> {
    // First check if already installed
    const check = await machineCheckPlannotator(machineId);
    if (check.installed) {
        return { success: true, message: `Plannotator ${check.version} already installed`, alreadyInstalled: true };
    }

    // Install using the official script (works on macOS and Linux)
    const installCmd = 'curl -fsSL https://plannotator.ai/install.sh | bash';
    const result = await machineBash(machineId, installCmd, '/tmp');

    if (result.success && result.exitCode === 0) {
        // Verify installation
        const verifyCheck = await machineCheckPlannotator(machineId);
        if (verifyCheck.installed) {
            return { success: true, message: `Plannotator ${verifyCheck.version} installed successfully` };
        }
