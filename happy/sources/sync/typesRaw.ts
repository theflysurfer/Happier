...
const rawAgentRecordSchema = z.discriminatedUnion('type', [z.object({
    ...
    type: z.literal('acp'),
    provider: z.enum(['gemini', 'codex', 'claude', 'opencode', 'pi']),
    data: z.discriminatedUnion('type', [
        // Core message types
        z.object({ type: z.literal('reasoning'), message: z.string() }),
        z.object({ type: z.literal('message'), message: z.string() }),
...
// Schema for image content block (Claude multimodal format)
...
// User content can be either simple text or multimodal (array of text/image blocks)
const rawUserContentSchema = z.union([
    ...
    // Multimodal format: array of content blocks
    z.array(z.union([
        z.object({
            type: z.literal('text'),
            text: z.string()
...
// User content can be text-only or multimodal (array with images and text)
type NormalizedUserContent = {
    type: 'text';
    text: string;
} | Array<{ type: 'text'; text: string; } | NormalizedImageContent>;
