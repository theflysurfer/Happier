...
export function getProfileEnvironmentVariables(profile: AIBackendProfile): Record<string, string> {
    ...
    // Add Plannotator config for remote sessions
    // These enable the Plannotator plan review UI for Claude Code sessions
    // Can be overridden by profile.environmentVariables if user sets them explicitly
    if (!envVars.PLANNOTATOR_REMOTE) {
        envVars.PLANNOTATOR_REMOTE = '1';
    }
    if (!envVars.PLANNOTATOR_PORT) {
        envVars.PLANNOTATOR_PORT = '19432';
    }
