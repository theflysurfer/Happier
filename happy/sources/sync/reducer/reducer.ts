...
export function reducer(state: ReducerState, messages: NormalizedMessage[], agentState?: AgentState | null): ReducerResult {
    ...
    for (let msg of nonSidechainMessages) {
        if (msg.role === 'user') {
            ...
            // Extract text from user content (can be simple text object or multimodal array)
            let userText: string;
            if (Array.isArray(msg.content)) {
                // Multimodal: find text blocks and join them
                const textBlocks = msg.content.filter((block): block is { type: 'text'; text: string } => block.type === 'text');
                userText = textBlocks.map(b => b.text).join('\n');
            } else {
                // Simple text content
