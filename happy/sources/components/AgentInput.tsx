...
export const AgentInput = React.memo(React.forwardRef<MultiTextInputHandle, AgentInputProps>((props, ref) => {
    ...
    // Command palette state - shows when user types "/" at the start
    const [showCommandPalette, setShowCommandPalette] = React.useState(false);
    const [commandPaletteQuery, setCommandPaletteQuery] = React.useState('');

    // Detect "/" at start of input to trigger command palette
    React.useEffect(() => {
        const text = inputState.text;
        // Show command palette when input starts with "/" and cursor is at or after the "/"
        if (text.startsWith('/') && inputState.selection.start >= 1) {
            setShowCommandPalette(true);
            // Extract the query (everything after "/")
            setCommandPaletteQuery(text.slice(1));
        } else {
            setShowCommandPalette(false);
            setCommandPaletteQuery('');
        }
    }, [inputState.text, inputState.selection.start]);

    // Handle command palette selection
    const handleCommandSelect = React.useCallback((command: UnifiedCommand, text?: string) => {
        if (text) {
            // For slash commands, set the text and send
            if (inputRef.current) {
                inputRef.current.setTextAndSelection(text, { start: text.length, end: text.length });
            }
            props.onChangeText(text);
            // Auto-send slash commands
            setTimeout(() => props.onSend(), 50);
        }
        setShowCommandPalette(false);
        setCommandPaletteQuery('');
        hapticsLight();
    }, [props.onChangeText, props.onSend]);
    ...
    const handleCloseCommandPalette = React.useCallback(() => {
        ...
        if (inputRef.current && inputState.text.startsWith('/')) {
            inputRef.current.setTextAndSelection('', { start: 0, end: 0 });
            props.onChangeText('');
        }
    }, [inputState.text, props.onChangeText]);
    ...
    const handleKeyPress = React.useCallback((event: KeyPressEvent): boolean => {
        // Handle command palette Escape first
    ...
    }, [showCommandPalette, handleCloseCommandPalette, suggestions, moveUp, moveDown, selected, handleSuggestionSelect, props.showAbortButton, props.onAbort, isAborting, handleAbortPress, agentInputEnterToSend, props.value, props.onSend, props.permissionMode, props.onPermissionModeChange]);
    ...
    return (
        <View style={[
            ...
            <View style={[
                ...
                {/* Command palette overlay - shows when "/" is typed */}
                {showCommandPalette && (
                    <>
                        <TouchableWithoutFeedback onPress={handleCloseCommandPalette}>
                ...
                {/* Autocomplete suggestions overlay - only show when not showing command palette */}
                ...
                <View style={styles.unifiedPanel}>
                    ...
                    {(props.images?.length || props.isImageProcessing) && (
                        <ImagePreviewRow
                            ...
                            isProcessing={props.isImageProcessing}
                        />
                    )}
