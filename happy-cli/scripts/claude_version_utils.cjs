...
function findClaudeInPath() {
    try {
        ...
        if (process.platform === 'win32') {
            // On Windows, 'where' returns multiple results. The npm wrapper
            // includes a POSIX shell script (no extension) that can't be spawned.
            // Pick the first directly-executable candidate.
            const lines = result.split('\n').map(l => l.trim()).filter(Boolean);
            claudePath = lines.find(l => {
                const lower = l.toLowerCase();
                return lower.endsWith('.exe') || lower.endsWith('.js') || lower.endsWith('.cjs');
            }) || null;
        } else {
...
function detectSourceFromPath(resolvedPath) {
    ...
    if (normalizedPath.includes('appdata') || normalizedPath.includes('program files') || normalizedPath.endsWith('.exe')) {
        ...
        // Windows native installer in AppData (exclude npm wrapper paths like AppData\Roaming\npm\claude)
        if (normalizedPath.includes('appdata') && normalizedPath.includes('claude') && !normalizedPath.includes('npm')) {
            return 'native installer';
        }
