...
(async () => {
  // Write PID file for daemon tracking (Windows Terminal spawn mode).
  // Must happen before anything else so the daemon can poll for it immediately.
  if (process.env.HAPPY_PID_FILE) {
    try {
      writeFileSync(process.env.HAPPY_PID_FILE, process.pid.toString())
    } catch {}
  }
  ...
  if (subcommand === 'doctor') {
    ...
    // Handle pi command (Pi coding agent — multi-provider, in-process SDK)
    try {
      ...
      // Auto-start daemon (same as claude/gemini)
...
${chalk.bold('Usage:')}
  ...
  happy pi                Start Pi mode (multi-provider, in-process)
