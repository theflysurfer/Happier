...
import { isWindowsTerminalAvailable, spawnInWindowsTerminal } from '@/utils/windowsTerminal';
  ...
  try {
    ...
    const spawnSession = async (options: SpawnSessionOptions): Promise<SpawnSessionResult> => {
      ...
      try {
        ...
        // Windows Terminal mode (Windows equivalent of tmux)
        // Auto-detected: if wt.exe is available on Windows, spawn in a new WT tab
        // instead of a detached headless console. Opt-out with HAPPY_HEADLESS_SESSIONS=true.
        if (!useTmux && process.platform === 'win32' && !process.env.HAPPY_HEADLESS_SESSIONS && isWindowsTerminalAvailable()) {
          logger.debug('[DAEMON RUN] Windows Terminal detected, spawning session in new tab');

          const agent = options.agent === 'gemini' ? 'gemini' : (options.agent === 'codex' ? 'codex' : 'claude');

          const wtResult = await spawnInWindowsTerminal({
            cliArgs: [agent, '--happy-starting-mode', 'remote', '--started-by', 'daemon'],
            directory,
            tabTitle: `Happy: ${basename(directory)}`,
            env: { ...process.env as Record<string, string>, ...extraEnv }
          });

          if (wtResult.success) {
            if (wtResult.pid) {
              logger.debug(`[DAEMON RUN] Windows Terminal tab opened, PID: ${wtResult.pid}`);
              ...
              const trackedSession: TrackedSession = {
                ...
                directoryCreated,
                message: directoryCreated
                  ? `Created '${directory}' and spawned session in Windows Terminal tab.`
                  : 'Spawned session in Windows Terminal tab.'
              };

              pidToTrackedSession.set(wtResult.pid, trackedSession);

              // Wait for webhook to populate session with happySessionId (same as tmux mode)
              logger.debug(`[DAEMON RUN] Waiting for session webhook for PID ${wtResult.pid} (Windows Terminal)`);

              return new Promise((resolve) => {
                const timeout = setTimeout(() => {
                  pidToAwaiter.delete(wtResult.pid!);
                  logger.debug(`[DAEMON RUN] Session webhook timeout for PID ${wtResult.pid} (Windows Terminal)`);
                  resolve({
                    type: 'error',
                    errorMessage: `Session webhook timeout for PID ${wtResult.pid} (Windows Terminal)`
                  });
                }, 15_000);

                pidToAwaiter.set(wtResult.pid!, (completedSession) => {
                  clearTimeout(timeout);
                  logger.debug(`[DAEMON RUN] Session ${completedSession.happySessionId} fully spawned with webhook (Windows Terminal)`);
                  resolve({
                    type: 'success',
                    sessionId: completedSession.happySessionId!
                  });
              ...
              // Tab opened but PID tracking failed. Session is likely running in the WT tab.
              // Do NOT fall through to regular spawning - that would create a duplicate session.
              logger.debug(`[DAEMON RUN] Windows Terminal tab opened but PID unknown. Waiting for session webhook by any PID.`);
              return new Promise((resolve) => {
                const timeout = setTimeout(() => {
                  logger.debug(`[DAEMON RUN] Session webhook timeout (Windows Terminal, no PID)`);
                  resolve({
                    type: 'error',
                    errorMessage: 'Session started in Windows Terminal tab but webhook timed out (PID unknown)'
                  });
                }, 30_000);

                // Since we don't know the PID, register a one-shot listener for any new session
                // from this directory. The webhook handler will match by happySessionId.
                const checkInterval = setInterval(() => {
                  // Check if any new tracked session appeared (from webhook)
                  for (const [, session] of pidToTrackedSession) {
                    if (session.happySessionId) {
                      clearTimeout(timeout);
                      clearInterval(checkInterval);
                      logger.debug(`[DAEMON RUN] Found session ${session.happySessionId} via polling (Windows Terminal, no PID)`);
                      resolve({
                        type: 'success',
            ...
            logger.debug(`[DAEMON RUN] Windows Terminal spawn failed: ${wtResult.error}, falling back to regular spawning`);
          }
        }

        // Regular process spawning (fallback or if tmux/WT not available)
        if (!useTmux) {
